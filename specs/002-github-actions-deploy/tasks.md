...
- [X] T035 SC-001 end-to-end validation: **v1.0.0 stable shipped to the Marketplace on 2026-05-17** via the full publish.yml pipeline. All 6 per-platform VSIXs published; GitHub Release v1.0.0 has all 6 attached with auto-generated changelog. The full publish run took ~3 minutes on GitHub-hosted runners (preflight → wait-ci → collision-check → package → publish → release-create). v0.7.0 also lives on the Marketplace pre-release channel (manually published during pipeline debugging; left in place as historical record of the test bed).
