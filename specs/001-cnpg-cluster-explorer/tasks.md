...
- [~] T153 **Partial — messaging shell shipped, glide-data-grid integration pending.** `src/webviews/grid/index.tsx` now wires the host's `init` → `loadPage` → `page` cycle end-to-end (React 18 + `createRoot`; receives messages via `window.addEventListener('message')`; posts `ready` on mount via `acquireVsCodeApi()`). Renders a basic HTML `<table>` of the loaded rows + a header showing `schema.table · kind · cluster/database · mode · PK columns · editable flag`, sticky header row with PK badge (🔑) and pg-type chip, NULL rendering with grayed-out italic, footer with row count. All styles use `var(--vscode-*)` CSS variables (no hard-coded hex). This shell validates the full CSP / bundle / protocol path end-to-end before T153's canvas-grid integration lands. Bundle weight grew from 138.7 KB → 141.0 KB. Glide-data-grid wiring (per-type cell editors via the T149 registry, sort/filter/hide/freeze UI in column headers, dirty-row tracking, Apply modal preview, FK-cell context menu, layout-change emit) is the remaining T153 work — the host (T150) already accepts every protocol message the renderer will send, so this is purely renderer-side polish.
