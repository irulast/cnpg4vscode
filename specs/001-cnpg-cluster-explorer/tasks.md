...
- [ ] T138 [P] Failing contract tests for the host↔grid message protocol at `test/contract/webview/grid-protocol.test.ts`. Covers: discriminated-union completeness (every `type` in the protocol contract has both a builder and a validator), unknown-type rejection at `warn`, payload-shape validation (numeric bounds, string non-empty, no embedded SQL), correlation-id round-tripping for request/response pairs.
...
- [ ] T140 [P] Failing unit tests for the per-type cell-editor registry at `test/unit/grid-cell-editors.test.ts`. Given a `ColumnDescriptor`, returns the correct editor kind (text / number / boolean / date / json / enum). Covers PG type → JS type mapping (`int4` → number, `jsonb` → json, `timestamptz` → date, `bool` → boolean, `text` → string, enum-typed → enum), nullable-bit precedence, default-bit precedence.
- [ ] T141 [P] Failing unit tests for the bulk-apply orchestration at `test/unit/grid-bulk-apply.test.ts`. Drives the per-row apply loop with injected callbacks (preview, confirm, executeStatement) and asserts: per-row dirty marks clear as each succeeds, a mid-set failure does NOT halt subsequent rows (each row is independent), the modal preview shows ALL statements before ANY runs, applying with zero dirty rows is a no-op.
...
- [ ] T149 Implement `src/grid/cell-editors.ts` — pure registry that maps a `ColumnDescriptor` to the glide-data-grid `GridCellKind` + editor configuration. Handles the PG-type → JS-type mapping; populates enum dropdown values from `pg_enum` at descriptor-load time (one query per enum-typed column, cached per session).
...
- [ ] T151 Implement the bulk-apply orchestration at `src/grid/bulk-apply.ts`. Pure wrapper around the per-row cell-edit-orchestrator that: builds all statements first, hands the full set to the preview modal, then iterates per-row with continue-on-failure semantics (a single bad row doesn't halt the rest). Streams `applyResult` outbound messages so the renderer can clear dirty marks incrementally.
- [ ] T152 Implement `buildInsert()` in `src/sql/update-builder.ts` (extends the existing module). Generates parameterised `INSERT INTO "s"."t" (cols) VALUES ($1, $2, ...) RETURNING ...`. Columns the user didn't specify go in as `DEFAULT`. Same NULL / DEFAULT sentinel handling as `buildUpdate()`.
