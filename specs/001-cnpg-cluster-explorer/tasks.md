...
- [ ] T150 Implement the host-side Grid Editor controller at `src/grid/host.ts`. Owns the `vscode.WebviewPanel`, builds the initial `init` message from the result-descriptor + layout state, handles inbound `loadPage` / `applyRequested` / `deleteRequested` / `insertRequested` / `openReferencedRow` / `layoutChanged` / `exportRequested` / `refreshRequested` messages per the protocol contract. Routes edit requests through the already-built `src/sql/cell-edit-orchestrator.ts` — no SQL composition in this module. Wires `connection.onModeChanged` → `modeChanged` outbound message. Persists `layoutChanged` payloads to `workspaceState` via the T147 serializer.
...
- [ ] T153 Implement the renderer bundle at `src/webviews/grid/{index.tsx,App.tsx,Grid.tsx,...}`. React + glide-data-grid. Owns the canvas grid, per-type cell editors via the T149 registry, sort/filter/hide/freeze UI in the column headers, dirty-row tracking, the Apply modal preview, the FK-cell context menu, the per-cluster persistence emit. Theme tokens fed from `--vscode-*` via the `init` message. No inline styles with hard-coded hex (T146 theme-snapshot enforces).
- [ ] T154 Implement the esbuild config update for `dist/webviews/grid/bundle.js`. Per the JSX target, treeshake React-DOM dev warnings out in production builds. Output a per-build content-hashed nonce-friendly script tag in `index.html`. Verify against `scripts/audit-webview-csp.mjs` (T130) — that script becomes load-bearing.
- [ ] T155 Wire the schema-tree menu entry: right-click a TABLE (or VIEW / MATERIALIZED VIEW) → **CNPG: Open in Grid Editor**. New `cnpg.grid.open` command in `src/commands/index.ts`, dispatching to `src/grid/host.ts:openGridEditor(node)`. Menu contribution in `package.json` `view/item/context` gated on `viewItem =~ /^(table|view|materializedView)$/` (no Write-mode gate — read-only mode still opens, just disables edit affordances).
- [ ] T156 Wire the command-palette entry: **CNPG: Open Table in Grid Editor...** — prompts the user to pick a connection, then a schema, then a table; opens the Grid Editor. Same dispatch as T155.
