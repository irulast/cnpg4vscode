...
- [ ] T136 [P] Add runtime dependencies: `@glideapps/glide-data-grid@^6`, `react@^18`, `react-dom@^18`. Update `THIRD_PARTY_LICENSES` (each is MIT). Add a `dist/webviews/grid/` esbuild target — separate bundle from `dist/extension.js` and `dist/notebook-renderer.js`. Activation path stays zero-cost because the grid bundle loads only when a panel opens. Update `pnpm audit:csp` to include the new directory.

### Tests for Phase 8.5 (TDD red → green)

- [ ] T137 [P] Failing unit tests for the persisted layout-state serializer at `test/unit/grid-editor-state.test.ts`. Covers: round-trip serialization, version-tag for forward-compat migration, defense-in-depth rejection of any field outside the enumerated allowlist (FR-039 "MUST NOT include cell data"), key shape `(context, ns, cluster, db, schema, table)` collision-free across overlapping contexts.
...
- [ ] T139 [P] Failing unit tests for the SELECT-builder driver at `test/unit/grid-select-builder.test.ts`. Given `(schema, table, sort?, filters?, limit, offset)`, produces a parameterized SELECT with quoted identifiers and a parallel `count(*)` for the totalRows display. Covers single + multi-column sort, every filter op (`eq/ne/lt/le/gt/ge/like/ilike/is_null/is_not_null`), trailing `LIMIT ... OFFSET ...`, defense against SQL-injection-shaped filter values.
...
- [ ] T147 Implement the persisted layout-state serializer at `src/state/grid-editor-state.ts`. Pure module — `serialize()` / `deserialize()` / `key()` / `applyDefaults(descriptor)`. Defense-in-depth field allowlist matches `test/unit/grid-editor-state.test.ts`'s assertions. Reads/writes via the existing `context.workspaceState` chokepoint (must be allow-listed in `eslint-rules/no-state-write-outside-history.cjs` or moved through `src/state/`).
- [ ] T148 Implement `src/sql/select-builder.ts` — pure parameterised SELECT composer consumed by the Grid Editor host's page-loading path. Quotes identifiers via `quoteIdent()`. Filter values bind as `$N`; filter `op` is matched against a frozen allowlist (no SQL injection surface). Includes a sibling `count(*)` builder for the totalRows display.
