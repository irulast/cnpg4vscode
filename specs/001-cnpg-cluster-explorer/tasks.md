...
- [-] T072 [P] [US4] Implement query history storage at `src/state/history.ts` — SQLite + FTS5. *(Deferred — better-sqlite3 native build setup adds complexity that benefits from its own focused pass. The redaction chokepoint (T016) and KV logging (T078) are in place so when history lands every entry already flows through redact().)*
...
- [ ] T095 [P] [US6] Write failing unit tests for the UPDATE/DELETE builder at `test/unit/update-builder.test.ts` — single-PK and composite-PK cases, NULL handling, type-correct casts.
- [ ] T096 [P] [US6] Write failing unit tests for PK detection from a SELECT AST at `test/unit/pk-detection.test.ts` — simple `SELECT … FROM t`, qualified `SELECT … FROM s.t`, joined SELECTs (must yield no PK), computed-column SELECTs (no PK).
- [ ] T097 [P] [US6] Write failing contract tests for the migration wizard's transactional execution at `test/contract/pg/migration.test.ts` — two-statement migration, second-statement-fails rollback, mixed transactional/non-transactional detection (`CREATE INDEX CONCURRENTLY`).
...
- [ ] T107 [P] [US6] Implement PK detection at `src/pg/result-descriptor.ts` consuming the `@pg-query/parser` AST of the executed SELECT to detect a single-source table, then issuing the result-set-descriptor query from contracts/pg-introspection.md to fetch PK columns.
- [ ] T108 [US6] Implement the UPDATE/DELETE builder at `src/sql/update-builder.ts` — generates parameterized statements; preview rendered as a `MarkdownString` before execution; reject if connection is not in Write mode or PK is missing.
- [ ] T109 [US6] Wire the cell-edit apply flow through the host (T105) — `applyRequested → editPreview → applyConfirmed | applyCancelled → editApplied | editFailed`.
...
- [ ] T112 [P] [US6] Implement the migration wizard webview at `src/ui/migration/host.ts` — collects ordered statements, detects non-transactional DDL via the AST parser, surfaces a warning.
- [ ] T113 [US6] Implement transactional execution at `src/sql/migration.ts` — `BEGIN; … ; COMMIT;` wrap when fully transactional; on failure ROLLBACK with the offending statement highlighted (FR-027).
- [ ] T114 [US6] Implement migration export-to-`.sql` at `src/sql/migration.ts` — writes the assembled statements (post-redaction) under `${scriptsRoot}` with a timestamped filename.
...
- [ ] T124 [P] Add an automated log-scrubber check at `test/e2e/log-scrubber.test.ts` asserting SC-006 (zero credentials/tokens/kubeconfig contents in the LogOutputChannel across the full test run).
...
- [ ] T126 [P] Add an SC-008 corpus check at `test/contract/pg/readonly-gate-corpus.test.ts` (≥50 representative statements: DML, DDL, transaction control, vendor-specific).
- [ ] T127 [P] Add an SC-009 destructive-confirmation check at `test/e2e/destructive-confirmation-corpus.test.ts` (every DROP/TRUNCATE/REINDEX/destructive-ALTER command requires typed-name confirmation; modal blocks until exact match).
...
- [ ] T129 [P] Implement `cnpg.history.open` and `cnpg.history.clear` commands at `src/commands/history.ts` — open routes to the panel webview from T077; clear requires a confirmation.
...
- [ ] T133 [P] Populate `THIRD_PARTY_LICENSES` with attribution for every runtime dep (constitution §Security final bullet).
