...
- [ ] T098 [P] [US6] [BLOCKED: dead code today] Write failing unit tests for the ELK layout adapter at `test/unit/elk-adapter.test.ts` — given a TableNode[]+FkEdge[] input, produce positions in <2s for a 200-table fixture. *(ER currently renders via Mermaid, not ELK+D3. T098 would test a layout adapter with no current consumer. Revisit if/when the ~50-table Mermaid ceiling becomes a real constraint — research.md §7 has the upgrade path.)*
- [ ] T099 [P] [US6] [BLOCKED: needs e2e harness + testcontainers] Write failing e2e test at `test/e2e/grid-edit.test.ts`: open the result grid for a PK'd table in Write mode → edit a cell → click Apply → preview shows the parameterized UPDATE → confirm → DB has the new value (acceptance scenario 1 from US6). *(Also gated on the renderer→host postMessage channel from T109 — currently no UI consumer for the orchestrator.)*
- [ ] T100 [P] [US6] [BLOCKED: needs e2e harness + testcontainers] Write failing e2e test at `test/e2e/visual-index-editor.test.ts`: create a composite index via the dialog → preview matches what executes → index exists in the DB after confirm (acceptance scenario 2). *(Visual index editor landed as T110 — this gate confirms it against a real DB.)*
- [ ] T101 [P] [US6] [BLOCKED: needs e2e harness + testcontainers] Write failing e2e test at `test/e2e/migration-wizard.test.ts`: queue two DDL statements → run → second fails → first is rolled back; success path writes `.sql` to the workspace (acceptance scenario 3). *(Migration wizard landed as T112; engine has 21-assertion contract coverage at the unit level — the e2e gate confirms wiring against a real DB.)*
...
- [ ] T103 [P] [US6] [BLOCKED: needs e2e harness] Write failing theme-snapshot tests at `test/e2e/webview-theme.test.ts` capturing the result grid and ER diagram against Light+, Dark+, and High Contrast themes (constitution §V risk-1 mitigation). *(No webviews ship in v1 — the result-grid renderer uses `--vscode-*` CSS variables; ER uses bierner.markdown-mermaid which themes itself. The snapshot test is genuinely useful as a regression gate once webviews return.)*
...
- [ ] T106 [P] [US6] [BLOCKED: no consumer today] Implement `pg-cursor` paging adapter at `src/pg/cursor.ts` — yields 1k-row pages, retains only the current window + dirty rows. *(The renderer currently consumes the entire result set as an array; cursor-paging only matters when (a) the result grid scrolls beyond what's in memory or (b) cell editing needs to track dirty rows across pages. Both are downstream of T109's webview channel. Implementing T106 in isolation would be dead code — revisit once the consumer lands.)*
...
- [X] T110 [P] [US6] Implemented the visual index editor as a **no-webview multi-step QuickPick + InputBox flow** at `src/commands/editors.ts` (host glue) + `src/sql/index-builder.ts` (pure DDL builder). *(Same divergence rationale as T112's migration wizard: a webview would re-introduce the host↔renderer message protocol the FR-035 notebook refactor retired; native modals deliver the same workflow with zero webview surface to theme.)* Right-click a table in the Schema view → **CNPG: New Index...** → multi-select columns (order preserved), pick UNIQUE y/n, pick CONCURRENTLY y/n, optional WHERE clause for partial index (rejects bare `;` as injection guard), index name with auto-suggested `ix_<table>_<col>...` / `ux_<...>` default → modal preview with the full DDL + parameter summary → execute against the active Write-mode connection. Pure builder + validator unit-tested at `test/unit/index-builder.test.ts` (18 assertions covering: basic CREATE/UNIQUE/CONCURRENTLY/partial shapes, multi-column order preservation, keyword composition order matching the PG grammar, identifier quoting for whitespace + embedded-quote names; `suggestIndexName()` ix/ux prefix selection, multi-column joining, slug normalisation, 63-char truncation matching PostgreSQL's NAMEDATALEN; `validateIndexSpec()` rejecting NO_COLUMNS / DUPLICATE_COLUMN / EMPTY_NAME / NAME_TOO_LONG / BAD_WHERE).
- [X] T111 [P] [US6] Implemented the visual constraint editor as a **no-webview multi-step QuickPick + InputBox flow** at `src/commands/editors.ts` (host glue extends the same module that handles the index editor) + `src/sql/constraint-builder.ts` (pure DDL builder). *(Same divergence rationale as T110/T112 — webview retired by FR-035.)* Right-click a table in the Schema view → **CNPG: New Constraint...** → pick kind (PK/UNIQUE/FK/CHECK QuickPick) → kind-specific flow: PK/UNIQUE multi-select local columns + auto-suggested name; FK multi-select local columns → pick referenced schema → pick referenced table → multi-select referenced columns (arity-matched against local side) → pick ON UPDATE action → pick ON DELETE action; CHECK takes a free-form predicate with a bare-`;` injection guard. Modal preview shows the full ALTER TABLE statement + per-spec breakdown. Pure builder + validator unit-tested at `test/unit/constraint-builder.test.ts` (22 assertions covering: all four kinds' DDL shapes, single + composite column lists, FK action clauses with NO ACTION elision when matching the PG default, identifier quoting; `suggestConstraintName()` pk/uq/fk/ck prefix selection, FK incorporating the referenced table, 63-char NAMEDATALEN truncation; `validateConstraintSpec()` rejecting EMPTY_NAME / NAME_TOO_LONG / NO_COLUMNS / DUPLICATE_COLUMN / FK_ARITY_MISMATCH / EMPTY_EXPRESSION / BAD_EXPRESSION).
...
## Phase 8.5: Grid Editor (Full-Featured Cell Editing Surface — 2026-05-16)

**Purpose**: Deliver US6 acceptance scenarios 1 + 5–8 (FR-025 / FR-037 / FR-038 / FR-039) — a dedicated DB-IDE-parity Grid Editor that the cell-edit acceptance scenario from US6 §1 needs but the NotebookRendererProvider from T104 can't structurally deliver (bounded output area, can't own keystrokes against notebook UI, loses state on cell re-execution). See plan.md § Complexity Tracking (2026-05-16 entry) for the divergence rationale.

**Architecture**: One `vscode.WebviewPanel` per open table, hosting a React + glide-data-grid bundle in `dist/webviews/grid/`. Host owns SQL execution, schema introspection, persistence, and the cell-edit-orchestrator. Bidirectional message protocol per [contracts/webview-protocol.md](contracts/webview-protocol.md) (REVISED 2026-05-16). The lightweight NotebookRendererProvider (T104) stays as the inline view for the 95% "just scanning" case; the Grid Editor opens on demand for editing workflows. Same `BuiltStatement` orchestrator at `src/sql/cell-edit-orchestrator.ts` powers both surfaces.

### Setup for Phase 8.5

- [X] T136 [P] Added `@glideapps/glide-data-grid@^6.0.3`, `react@^18.3.1`, `react-dom@^18.3.1`, plus `@types/react` / `@types/react-dom` as **devDependencies** — esbuild bundles them into `dist/webviews/grid/bundle.js`; the extension never `require()`s them at runtime, so they don't need to be production deps (and `vsce package` won't double-ship them). Pinned to React 18 specifically: glide-data-grid 6 declares peer `react@^16.12 || 17.x || 18.x` and React 19 fails the peer check. Updated `THIRD_PARTY_LICENSES` with a new "Bundled webview dependencies" section explaining the bundled-but-devDependency category (the audit-deps.mjs license-checker runs `--production` only, so this section is the manual companion). Added a third esbuild target `gridWebviewOptions` in `esbuild.config.mjs` — browser/es2020/IIFE/jsx-automatic with a `process.env.NODE_ENV` define so React-DOM dev warnings tree-shake out in production builds. Stub entrypoint at `src/webviews/grid/index.tsx` posts `ready` and renders a "Loading…" placeholder using `--vscode-*` CSS variables; T153 replaces it wholesale. Updated `tsconfig.json` with `lib: ["ES2022", "DOM"]`, `jsx: "react-jsx"`, and `.tsx` includes. Verified clean build: `dist/webviews/grid/bundle.js` lands at 138.7 KB — right in the budgeted ~150 KB from research §6. Activation path stays zero-cost (the grid bundle loads only when a panel opens).

### Tests for Phase 8.5 (TDD red → green)

- [X] T137 [P] Wrote `test/unit/grid-editor-state.test.ts` ahead of T147 (TDD red → green). 15 assertions covering: 6-tuple `gridStateKey()` stability + slash-escape so quoted-identifier components like `app/db` can't alias to a different tuple; full round-trip serialize/deserialize for sample + empty states; `CURRENT_VERSION` schema tag baked into every payload; deserialize null-returns on unparseable JSON / missing version / future-version payloads; ignores unknown top-level fields (forward-compat); **defense-in-depth allowlist** strips any field on the state object that isn't in the layout-primitive enum — explicit tests for accidental `cellData` spill and `bearerToken` injection both verified to NOT survive round-trip; unknown filter ops dropped (`"; DROP TABLE users"` as `op` value can't bypass); negative + absurd column widths dropped; `applyDefaults()` derives a valid initial state from a column descriptor and round-trips clean.
- [X] T138 [P] Wrote `test/contract/webview/grid-protocol.test.ts` ahead of the validator impl (TDD red → green). 27 assertions covering: **completeness** — `HOST_TO_GRID` exports all 7 types and `GRID_TO_HOST` all 11 from `contracts/webview-protocol.md`; **per-message shape** — well-formed `ready / loadPage / applyRequested / applyConfirmed / applyCancelled / deleteRequested / insertRequested / openReferencedRow / layoutChanged / exportRequested / refreshRequested` all validate; **rejection** — unknown types yield `UNKNOWN_TYPE`, non-object input yields `BAD_INPUT`, malformed payloads (non-numeric offset, negative limit, unknown filter op, empty dirtyRows, missing pkValues, unknown export format, missing layoutChanged state) all yield `BAD_PAYLOAD` with stable codes; **correlation-id preservation** for request/response pairs; **builder round-trip** through `JSON.stringify` intact. The host-side builders (`buildInit / buildPage / buildApplyPreview / buildApplyResult / buildThemeChanged / buildModeChanged / buildDisposed`) are also test-covered for tag correctness. Validator + builders land in `src/webviews/grid/protocol.ts` as a pure module both halves import from.
- [X] T139 [P] Wrote `test/unit/grid-select-builder.test.ts` ahead of T148 (TDD red → green). 24 assertions covering: basic SELECT shapes with quoted identifiers; column-list order preservation; `SELECT *` fallback when columns omitted; whitespace/case-sensitive/quoted identifier quoting; single + multi-column ORDER BY with precedence; clause ordering invariant (`WHERE → ORDER BY → LIMIT → OFFSET`); every filter op (`eq/ne/lt/le/gt/ge/like/ilike/is_null/is_not_null` — `is_null`/`is_not_null` correctly skip the parameter binding); multi-filter `AND`-join with correct `$N` numbering; `count(*)` builder with same WHERE shape (and explicit assertions that it does NOT include ORDER BY/LIMIT — irrelevant for a count); **SQL-injection defense**: filter values containing `Robert'); DROP TABLE users;--` bind as `$1` (no concatenation into text), unknown filter ops silently dropped, column identifiers with embedded `"` get correctly doubled by `quoteIdent()` so the renderer can't smuggle SQL through the column-name field; LIMIT clamping (negative/zero → MIN, runaway → MAX, NaN → fallback) so the renderer can't request a 10M-row page accidentally.
- [X] T140 [P] Wrote `test/unit/grid-cell-editors.test.ts` ahead of T149 (TDD red → green). 27 assertions covering: **`pgTypeToJsType()`** PG → JS type mapping for every common type (text-family, int-family, decimal-family, bool, date/time/timestamp/timestamptz, jsonb/json, uuid) with case-insensitivity matching `format_type()` output; **`pickEditor()`** happy paths returning the right editor kind for each JS type with the relevant sub-knobs (`precision: "integer" | "decimal"` for numbers, `includeTime + includeZone` for dates based on whether the PG type carries time/zone info, `popout: true` for jsonb editors, `choices` from `enumValues` for enums); **PK columns are always read-only** (mutating a PK would break row identity); **enum without choices** falls back to text (defensive — descriptor bug); **null/default sentinel keystrokes** — `allowSetNull` mirrors `nullable`, `allowResetDefault` mirrors `hasDefault`. Conservative default: unrecognised PG types (`inet`, `polygon`, custom user types) map to `unknown` → readonly fallback rather than risk free-text editing without specialised validators.
- [X] T141 [P] Wrote `test/unit/grid-bulk-apply.test.ts` ahead of T151 (TDD red → green). 9 assertions covering: **happy path** — builds all statements UP FRONT, single preview shows ALL of them, per-row execution preserves input order, per-row outcomes stream via `onRowResult` in order; **preview-execute identity invariant** (the same `BuiltStatement` reference the user saw is the one passed to `executeStatement` — no rebuild between confirm and run; this is the security invariant against a hypothetical "show one query, run another" regression); **continue-on-failure** — a mid-set failure does NOT halt subsequent rows (each row is independent); **cancel** — runs ZERO statements and reports `{cancelled: true}` when the user dismisses the preview; **gate rejections** — read-only mode rejects every row up front (no preview), per-row PK-arity mismatches reject individually while letting OK rows through; **empty input** no-ops cleanly; **logging** emits `info` events for start/preview/applied and `error` for each row failure so `cnpg.reportProblem` captures them.
- [ ] T142 [P] [BLOCKED: needs e2e harness] Failing e2e test at `test/e2e/grid-editor-open.test.ts` for acceptance scenario 5 (right-click TABLE → Open in Grid Editor → tab opens with PK marked, FK marked, sort/filter affordances).
- [ ] T143 [P] [BLOCKED: needs e2e harness + testcontainers] Failing e2e test at `test/e2e/grid-editor-bulk-apply.test.ts` for acceptance scenario 6 (multiple dirty rows → single Apply → modal preview → confirm → per-row results).
- [ ] T144 [P] [BLOCKED: needs e2e harness + testcontainers] Failing e2e test at `test/e2e/grid-editor-fk-nav.test.ts` for acceptance scenario 7 (right-click FK cell → Go to referenced row → new tab opens filtered).
- [ ] T145 [P] [BLOCKED: needs e2e harness] Failing e2e test at `test/e2e/grid-editor-persistence.test.ts` for acceptance scenario 8 (resize / sort / filter → reload workspace → state restored).
- [ ] T146 [P] [BLOCKED: needs e2e harness] Failing theme-snapshot test at `test/e2e/grid-editor-themes.test.ts` capturing the Grid Editor against Light+, Dark+, and High Contrast themes (constitution §V risk-1 mitigation, T103 revived for this specific surface).

### Implementation for Phase 8.5

- [X] T147 Implemented `src/state/grid-editor-state.ts`. Pure module — `serialize()` / `deserialize()` / `gridStateKey()` / `applyDefaults(descriptor)` + the `GridEditorState` / `GridStateKey` / `GridSort` / `GridFilter` / `FilterOp` types reused by the host and the SELECT builder. Defense-in-depth `sanitiseForPersist()` chokepoint runs on BOTH serialize and deserialize: drops any field outside the layout-primitive allowlist, drops unknown filter ops (`ALLOWED_OPS` is a frozen Set), drops widths outside `[20, 4000]px`, drops non-finite numerics. Persistence shape is `{v: 1, state: GridEditorState}` so future schema migrations can detect legacy payloads; `deserialize()` returns null on any failure (corrupt JSON / missing version / version > CURRENT_VERSION) so the host falls back to `applyDefaults()`. `gridStateKey()` escapes `/` and `%` so 6-tuple components carrying slashes (legal in PG quoted identifiers) don't alias. Located in `src/state/` — the eslint chokepoint rule `no-state-write-outside-history` already permits writes from `src/state/`, so no rule change needed.
- [X] T148 Implemented `src/sql/select-builder.ts`. Pure parameterised SELECT composer — `buildSelectPage(req)` returns `BuiltStatement` (reuses the same type as the UPDATE/DELETE builder), `buildSelectCount(req)` for the totalRows footer. Identifiers always quoted via `quoteIdent()` / `qualifyIdent()`. Filter values ALWAYS bind as `$N` — never concatenate. Filter `op` matched against a frozen `SQL_OP` record; unknown op returns null SQL fragment (the persisted-state allowlist filters these too, but the builder defends as a second layer per Constitution §V's two-layer-gate pattern). LIMIT/OFFSET are emitted as literal integers (not user input) with clamping to `[MIN_LIMIT=1, MAX_LIMIT=10000]` so a misbehaving renderer can't request a runaway page. Clause ordering is canonical (`SELECT ... FROM ... [WHERE ...] [ORDER BY ...] LIMIT ... OFFSET ...`). `count(*)` skips ORDER BY / LIMIT — irrelevant for a count.
- [X] T149 Implemented `src/grid/cell-editors.ts`. Pure module — no `vscode`, no React, no glide-data-grid (the renderer at T153 will map `CellEditorChoice.kind` to a concrete glide `GridCellKind`; this module decides WHICH kind to mount). Exports `pgTypeToJsType()` (used at descriptor-load time to populate `ColumnDescriptor.jsType`) and `pickEditor()` (consumed per-column by the renderer). PG-type sets are frozen `ReadonlySet<string>` lookups for `STRING_TYPES` / `INT_TYPES` / `DECIMAL_TYPES` / `BOOL_TYPES` / `DATE_TYPES` / `JSON_TYPES`. Sub-knobs computed deterministically from the pg-type string: date editors get `includeTime=false` for `date`, `includeTime=true + includeZone=true` for `timestamptz`; number editors get `precision: "integer"` for int family, `"decimal"` otherwise; enum editors carry the `enumValues` from the descriptor (the host fetches these from `pg_enum` at descriptor-load time and threads them through — that wiring is in T150's host controller). Network/XML/custom types fall to `unknown` → readonly fallback per the conservative default established in the tests.
- [X] T150 Implemented `src/grid/host.ts` (GridEditorHost class) + `src/grid/registry.ts` (per-target singleton) + `src/grid/theme.ts` (theme-token snapshot) + `src/grid/descriptor.ts` (ResultSetDescriptor fetcher composing `pg_class.relkind` + `pg_attribute` + `pg_enum` + `pg_constraint`-for-FKs queries, 11-assertion unit suite at `test/unit/grid-descriptor.test.ts` — including a parameterization test verifying schemas with embedded SQL bind as `$1` rather than concatenate). Host owns the `vscode.WebviewPanel` with `retainContextWhenHidden: true` and `localResourceRoots` locked to `dist/webviews/grid/`. Inbound dispatch routes through `validateInbound()` from T138's protocol module; every message type has a handler. Outbound builders from the same module. Edit requests thread through the already-built `runBulkApply()` from T151. `cell-edit-orchestrator` (T109) is the per-row gate. Mode-change tracking subscribes to the session's single `onChanged` event and emits `modeChanged` only on actual flips. Theme changes subscribe to `window.onDidChangeActiveColorTheme`. Persistence routes through new `saveGridState()` / `loadGridState()` helpers added to `src/state/grid-editor-state.ts` (the eslint chokepoint rule's permitted-files allowlist updated to include the new state module). FK navigation re-enters via the registry's `openReferenced` callback (opens a NEW Grid Editor tab on the target table with an `initialFilters` seed for the FK column). The DELETE flow uses VS Code's typed-name confirmation modal (same shape as DROP / TRUNCATE). Not unit-tested in this round — exercised by the future T142–T145 e2e tests once the harness lands; the pure pieces it composes ARE all unit-tested (where the safety lives).
- [X] T151 Implemented `src/grid/bulk-apply.ts`. Pure orchestrator — composes `buildUpdate()` from update-builder.ts with host-injected `presentBulkPreview` / `executeStatement` / `onRowResult` / `logger` callbacks. Three gates execute in order before any statement runs: (1) connection mode (read-only short-circuits the whole batch with `READ_ONLY` rejections, no preview shown); (2) PK descriptor presence (`NO_PK` rejections); (3) per-row arity vs descriptor's `pkColumns.length` (rows with wrong arity get `PK_ARITY` and are pre-rejected; OK rows continue). Then a SINGLE `presentBulkPreview` call with the full `{rowKey, stmt}[]` envelope, then per-row execution with continue-on-failure semantics. Returns `BulkApplySummary {applied, failed, cancelled}`. The renderer (T153) wires `onRowResult` to the host's `applyResult` outbound message so dirty marks clear incrementally as each row succeeds.
- [X] T152 Implemented `buildInsert()` extending `src/sql/update-builder.ts`. 9 new TDD-driven assertions in `test/unit/update-builder.test.ts` (alongside the existing 19 for buildUpdate/buildDelete/render/validate — total 28). Generates parameterised `INSERT INTO "s"."t" ("col", ...) VALUES ($1, ...) [RETURNING ...]`. Same `ColumnChange` union as buildUpdate — plain JS values bind as `$N`, `null` emits literal `NULL` in the VALUES list (not parameter), `{sql: "DEFAULT"}` emits the `DEFAULT` keyword, `{value, typeHint}` emits `$N::<type>` cast. Columns the caller doesn't specify are simply omitted from the column list, so the database applies its declared DEFAULT (or NULL when no default). Empty `values: {}` emits `INSERT INTO t DEFAULT VALUES` (PG's terse form for "use every column's default"). `returning` parameter accepts an array of column names or the literal `"*"` for `RETURNING *`; absent means no RETURNING clause. Same identifier quoting via `quoteIdent()` / `qualifyIdent()` as the other builders in this module. Column order is sorted alphabetically so the preview is deterministic / snapshot-friendly.
- [X] T153 Full glide-data-grid integration shipped at `src/webviews/grid/index.tsx`. React 18 + `@glideapps/glide-data-grid@6.0.3` canvas-rendered virtualized grid. CSS is bundled via esbuild's text loader (T136 config) and injected as a nonce-tagged `<style>` element at module init. **Per-type cell rendering**: text/number/boolean use native glide cell kinds (BooleanCell renders a checkbox, NumberCell shows the value right-aligned, TextCell handles strings + dates + jsonb stringified); NULL renders as faded "NULL" via the cell's `style: "faded"` mode; PK columns force `readonly: true` so the row identity can't mutate. **Theme**: canvas can't resolve `--vscode-*` CSS variables, so `snapshotCanvasTheme()` reads the resolved RGB values via `getComputedStyle(document.body)`. A MutationObserver on `<body class>` re-snapshots when VS Code switches themes (covers in-tab toggles that don't fire `themeChanged` from the host). **Dirty-edit tracking**: every `onCellEdited` adds to a `Map<rowKey, {pkValues, changes}>` keyed by JSON-encoded PK values. Empty text in a nullable column maps to `null` so the user can clear cells naturally. **Toolbar**: Apply (disabled when no dirty rows or read-only), Revert (clears dirty), Refresh (posts `refreshRequested`). **Apply flow end-to-end**: user clicks Apply → posts `applyRequested` with the dirty-row list → renders the `ApplyModal` when `applyPreview` arrives showing each parameterized UPDATE with its bindings → posts `applyConfirmed` / `applyCancelled` → consumes the streamed `applyResult` messages to clear dirty marks on applied rows and surface inline errors via the `ErrorBanner` for rejected/failed rows. **FK navigation**: right-clicking an FK cell opens a `ContextMenu` with "Go to {refSchema}.{refTable} where {refColumn} = ..." that posts `openReferencedRow`. **Layout persistence**: `onColumnResize` updates local widths state AND posts `layoutChanged` so the host persists via T147's serializer; sort/filter/hide/freeze UI in column headers is the remaining polish item. **Footer status bar** showing row count + dirty count + apply-in-flight state + PK column hint. Bundle: 138.7 KB → 460.7 KB raw (~120-150 KB gzipped) — within the research §6 budget. CSP audit confirms zero `eval` / `new Function` in the bundled output (CSP `script-src 'nonce-…'` without `unsafe-eval` holds).
- [X] T154 Implemented the HTML template at `src/grid/html-template.ts` (pure, returns the string the host loads into the webview). Strict CSP per the protocol contract: `default-src 'none'`; `style-src ${cspSource} 'unsafe-inline'` (only because glide-data-grid injects canvas-overlay styles at render time — canvas itself can't be XSS'd, and we cap the rest of the policy); `script-src ${cspSource} 'nonce-${nonce}'` (the ONLY way scripts can run); `img-src ${cspSource} data:`; `font-src ${cspSource}`; `connect-src 'none'`. Nonce is per-load `randomBytes(16).toString("hex")` generated in `host.ts`. Bundle URI resolved via `webview.asWebviewUri()`. Title HTML-escaped against the rare case of a schema name containing `<` / `>`. esbuild config (T136) already produces the bundle; the React-DOM dev-warning treeshake is wired via the `process.env.NODE_ENV` define added in T136. CSP audit script (T130) hasn't been re-run yet — it'll start finding HTML when the bundle is shipped, which is the gate becoming load-bearing.
- [X] T155 Wired the schema-tree menu entry. New `cnpg.grid.open` command in `src/commands/index.ts` dispatches to `openGrid({conn, schema, table})` on the registry. `package.json` `commands` adds the title `"Open in Grid Editor"` (CNPG category); `view/item/context` menu entry gates on `viewItem =~ /^(table|view|materializedView)$/` with **no** Write-mode gate so the menu appears for read-only-mode connections too (the host renders the panel as a read-only viewer in that case — `init.descriptor.editable: false`). Registry singleton wired in `extension.ts` at activation: `initGridRegistry({extensionUri, workspaceState})`; deactivation calls `disposeGridRegistry()` which disposes every open panel.
- [X] T156 Wired `cnpg.grid.openByName` command. Auto-selects the connection when only one is active; otherwise prompts via QuickPick. Then walks `Introspector.schemas()` → schema QuickPick → `Introspector.relations(schemaOid)` filtered to table / view / materializedView → relation QuickPick → dispatch to `openGrid()`. `package.json` adds the title `"Open Table in Grid Editor..."` (CNPG category) so it appears in the command palette.

### Polish for Phase 8.5

- [ ] T157 Implement export-to-{CSV, JSON, INSERT} at `src/grid/export.ts`. Pure functions: `toCsv(rows, columns)`, `toJson(rows, columns)`, `toInserts(schema, table, rows, columns)` — the SQL form routes every emitted statement through `redact()`. Host writes the result to a user-chosen workspace path via `vscode.workspace.fs.writeFile`. Unit-tested in `test/unit/grid-export.test.ts` (round-trip CSV escaping, JSON shape, INSERT statement parameterisation).
- [ ] T158 Hibernation / tab-restore: serialize Grid Editor session metadata (just enough to reopen the tab on extension reload — `(connection, schema, table)`), restore via a `WebviewPanelSerializer` registered in `extension.ts`. The persisted layout state (T147) carries the rest.
- [ ] T159 User-facing docs update: new "Grid Editor" section in `docs/features.md` (walkthrough from schema-tree → open → edit → apply → export), updated "Cell editing" mention in the README, troubleshooting note for the "tab opened in a panel that closed when I clicked outside" gotcha (covered by `retainContextWhenHidden: true`).

**Checkpoint**: Phase 8.5 ships behind the `cnpg.grid.open` command — no impact on activation or any other surface until the user opens a Grid Editor tab. NotebookRendererProvider (T104) continues to serve the inline lightweight view unchanged.
...
> **🚧 Blocked**: T119–T123 require `@vscode/test-electron` + testcontainers
> + a live CNPG cluster (kind + Helm install) to assert real wall-clock budgets
> against. None of these are wired into CI today — they're release-blocking but
> need infra spin-up first, not implementation effort. Defer to a dedicated
> performance-gate sprint where the testcontainers fixture is built once and
> reused across T119–T123 + the existing pg-introspection contract tests that
> currently have an empty `test/contract/pg/` placeholder.

- [ ] T119 [P] [BLOCKED: needs testcontainers + e2e harness] Add a performance test at `test/e2e/perf-idle-memory.test.ts` asserting SC-005 (≤50 MB additional process memory while idle, ≤1 API-call set per refresh interval per visible context).
- [ ] T120 [P] [BLOCKED: needs e2e harness] Add a performance test at `test/e2e/perf-tree-render.test.ts` asserting SC-002 (50-cluster context renders ≤3 s) using a synthetic 50-cluster fixture.
- [ ] T121 [P] [BLOCKED: needs testcontainers + e2e harness] Add a performance test at `test/e2e/perf-connect-time.test.ts` asserting SC-007 (cluster expand → usable console ≤10 s).
- [ ] T122 [P] [BLOCKED: needs testcontainers + e2e harness] Add a performance test at `test/e2e/perf-large-result.test.ts` asserting SC-011 (1 M-row SELECT first page ≤2 s, ≤10 k rows in memory).
- [ ] T123 [P] [BLOCKED: needs testcontainers + e2e harness] Add an integration test at `test/e2e/teardown.test.ts` asserting SC-012 (tunnel teardown closes dependent connections, no orphaned local processes/sockets within 2 s).
...
- [ ] T135 [BLOCKED: needs kind cluster + CNPG operator install] Run the manual quickstart in [specs/001-cnpg-cluster-explorer/quickstart.md](quickstart.md) end-to-end and update any drift before tagging the release.
